...
<body class="bg-[#0C0C0D] text-white min-h-screen">
    ...
    // Track if any items in each folder match the search
    const folderHasMatch = new Map();
    
    // First pass: Check all chat items and mark matching folders
    items.forEach(item => {
        if (item.classList.contains('chat-item')) {
            ...
            // Show/hide the chat item
            item.style.display = match ? '' : 'none';
            
            // If this item matches, mark all parent folders as having a match
            if (match) {
                let parent = item.parentElement;
                while (parent) {
                    if (parent.classList.contains('folder-item')) {
                        folderHasMatch.set(parent, true);
                    }
    ...
    items.forEach(item => {
        if (item.classList.contains('folder-item')) {
            ...
            const folderMatch = folderTitle.includes(query);
            const hasMatchingChildren = folderHasMatch.get(item);
            
            // Show folder if it matches the search or has matching children
            item.style.display = (folderMatch || hasMatchingChildren) ? '' : 'none';
            
            // If folder matches, show all parent folders
            if (folderMatch) {
                let parent = item.parentElement;
                while (parent) {
                    if (parent.classList.contains('folder-item')) {
            ...
            // If folder is visible, ensure its content div is visible
            const contentDiv = item.querySelector('div:nth-child(2)');
            if (contentDiv && (folderMatch || hasMatchingChildren)) {
