    ...
    <script>
        class ChatUI {
            ...
            createChatListItem(item, level = 0) {
                ...
                if (item.type === 'folder') {
                    ...
                    itemDiv.className = `folder-item ${indentClass}`;
                    itemDiv.setAttribute('data-folder-id', item.id);

                    itemDiv.innerHTML = `
            <div class="folder-header flex items-center justify-between p-2 text-zinc-400 text-sm hover:bg-zinc-800/60 rounded-lg cursor-pointer group">
                <div class="flex items-center flex-1">
                    <svg class="folder-arrow w-4 h-4 mr-2 transform transition-transform ${item.collapsed ? '-rotate-90' : ''}" 
                    ...
                    <span class="folder-title flex-1">${item.title}</span>
                </div>
                <div class="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button class="rename-folder p-1 hover:bg-zinc-700 rounded">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" 
                                  stroke-linecap="round" stroke-linejoin="round" stroke-width="2"/>
                        </svg>
                    </button>
                    <button class="pin-folder p-1 hover:bg-zinc-700 rounded">
                        <svg class="w-4 h-4 ${item.pinned ? 'text-yellow-500' : 'text-zinc-400'}" 
            ...
            <div class="folder-content ml-4 ${item.collapsed ? 'hidden' : ''}"></div>
        `;
                    ...
                    // Event Handlers for Folder
                    const folderHeader = itemDiv.querySelector('.folder-header');
                    const folderArrow = itemDiv.querySelector('.folder-arrow');
                    const renameButton = itemDiv.querySelector('.rename-folder');
                    const pinButton = itemDiv.querySelector('.pin-folder');
                    ...
                    // Create chat item structure
                    itemDiv.className = `chat-item group p-3 hover:bg-zinc-800/60 rounded-lg cursor-pointer transition-all ${indentClass} ${item.pinned ? 'border-l-2 border-yellow-500' : ''}`;
                    itemDiv.setAttribute('data-chat-id', item.id);
            ...
            <div class="flex justify-between items-start">
                ...
                <div class="chat-item-icons opacity-0 group-hover:opacity-100 transition-opacity flex gap-2">
                    <button class="rename-button p-1 hover:bg-zinc-700 rounded">
                    ...
                    <button class="tag-button p-1 hover:bg-zinc-700 rounded">
                        <svg class="w-4 h-4 text-zinc-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path d="M7 7h.01M7 3h5l7 7-7 7-7-7V7a4 4 0 014-4z" 
                                  stroke-linecap="round" stroke-linejoin="round" stroke-width="2"/>
                        </svg>
                    </button>
        ...
        `;
                    ...
                    chatContent.addEventListener('click', () => {
                        this.loadChat(item.id, item.title);
                    });
